...
import { searchBooks, transformGoogleBookToBook } from "@/lib/google-books";
...
export async function addBookToList(bookId: string, listType: BookListType) {
  ...
  try {
    ...
    if (existingBook.length === 0) {
      ...
      const googleBook = await searchBooks(`id:${bookId}`);
      if (googleBook.length === 0) {
        throw new Error("Book not found");
      }

      const bookData = transformGoogleBookToBook(googleBook[0]);
    ...
    if (existingUserBook.length > 0) {
      ...
      await db
        ...
        .where(eq(userBooks.id, existingUserBook[0].id));
    } else {
      // Add new entry
...
export async function removeBookFromList(bookId: string) {
  ...
  try {
    await db
      .delete(userBooks)
      .where(
        and(eq(userBooks.user_id, user.id), eq(userBooks.book_id, bookId)),
      );
      ...
      .where(
        listType
          ? and(
              ...
              eq(userBooks.list_type, listType),
            )
          : eq(userBooks.user_id, user.id),
      );
...
export async function getBookReviews(bookId: string) {
  ...
  try {
    const bookReviews = await db
      .select()
      .from(reviews)
      .where(eq(reviews.book_id, bookId));
